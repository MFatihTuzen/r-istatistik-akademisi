...
-   💡 **Büyük/Küçük Harf Duyarlılığı**\
    `x` ve `X` **farklı** nesnelerdir. Yanlış yazım hatalarını önlemek için dikkat edin.

-   💡 **Nesne Adlarında Türkçe Karakterlerden Kaçının**\
    `ö, ü, ğ, ç` gibi karakterler bazı ortamlarda sorun çıkarabilir.

-   💡 **Boşluk Kullanmayın**\
    `my data` yerine `my_data` veya `myData` gibi isimlendirmeler kullanın.

-   💡 **Çalışma Dizininizi Kontrol Edin**\
    Kodun başında `getwd()` ile doğru dizinde olduğunuzdan emin olun.

-   💡 **R Script Kaydetmeyi Unutmayın**\
    Script üzerinde çalışırken sık sık `Ctrl + S` (Windows) veya `Cmd + S` (Mac) ile kaydedin.
